...
# Note: We use a DO block for cleaner logic, but simple "|| true" works too.
run_sql_as_super "
DO \$\$
BEGIN
...
if ! PGPASSWORD=$DB_PASS psql -p "$DB_PORT" $HOST_ARG -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1 as connected;" &>/dev/null; then
    ...
    # We don't exit here because sometimes local socket connection differs slightly, 
    # but the URL generation below might still be correct.
else 
